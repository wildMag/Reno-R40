...
$.fn.extend($.ui.ariaSorTable.prototype,{
	...
	buildPager: function () {		
		...
		while (y < options.tableData.length){
			pagestart = (options.rowsToShow*site)+1;
			pageend = (options.rowsToShow*site)+options.rowsToShow;
			html += '	<button title="'+options.textPager+' ['+ pagestart +' - '+ pageend +']" type="button" class="ui-state-default ui-corner-all" aria-selected="false" aria-labelledby="ui-table-'+options.uid+'-pager-title">['+ pagestart +' - '+ pageend +']</button>'+"\n";
			site++;
